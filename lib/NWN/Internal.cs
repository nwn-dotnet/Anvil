...
namespace NWN
{
  internal class Internal
  {
    ...
    private static AllHandlers handlers;

    public static void RegisterHandlers(AllHandlers handlers)
    {
      Internal.handlers = handlers;
      int size = Marshal.SizeOf(typeof(AllHandlers));
      IntPtr ptr = Marshal.AllocHGlobal(size);
      Marshal.StructureToPtr(Internal.handlers, ptr, false);
      NativeFunctions.RegisterHandlers(ptr, (uint) size);
      Marshal.FreeHGlobal(ptr);
    }

    public static int Bootstrap(IntPtr arg, int argLength, AllHandlers handlers)
    {
      Log.Info("Initializing bootstrap structure");

      if (arg == (IntPtr) 0)
      {
        Log.Fatal("Received NULL bootstrap structure");
        return 1;
      }
      ...
      {
        Log.Fatal($"Received bootstrap structure too small - actual={argLength}, expected={expectedLength}");
        return 1;
      }

      if (argLength > expectedLength)
      {
        Log.Warn($"WARNING: Received bootstrap structure bigger than expected - actual={argLength}, expected={expectedLength}");
        Log.Warn($"         This usually means that native code version is ahead of the managed code");
      }
