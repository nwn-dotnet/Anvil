...
namespace Anvil.API
{
  public abstract partial class NwObject
  {
    ...
    internal static NwObject? CreateInternal(ICGameObject? gameObject)
    {
      ...
      {
        CNWSArea area => NwArea.CreateInternal(area),
        CNWSAreaOfEffectObject areaOfEffect => NwAreaOfEffect.CreateInternal(areaOfEffect),
        CNWSCreature creature => NwCreature.CreateInternal(creature),
        CNWSDoor door => NwDoor.CreateInternal(door),
        CNWSEncounter encounter => NwEncounter.CreateInternal(encounter),
        CNWSItem item => NwItem.CreateInternal(item),
        CNWSPlaceable placeable => NwPlaceable.CreateInternal(placeable),
        CNWSSoundObject soundObject => NwSound.CreateInternal(soundObject),
        CNWSStore store => NwStore.CreateInternal(store),
        CNWSTrigger trigger => NwTrigger.CreateInternal(trigger),
        CNWSWaypoint waypoint => NwWaypoint.CreateInternal(waypoint),
    ...
    private static NwObject? CreateFromVirtualType(ICGameObject gameObject)
    {
      ...
      {
        ObjectType.Creature => NwCreature.CreateInternal(CNWSCreature.FromPointer(NWNXUtils.AsNWSCreature(gameObject.Pointer))),
        ObjectType.Item => NwItem.CreateInternal(CNWSItem.FromPointer(NWNXUtils.AsNWSItem(gameObject.Pointer))),
        ObjectType.Placeable => NwPlaceable.CreateInternal(CNWSPlaceable.FromPointer(NWNXUtils.AsNWSPlaceable(gameObject.Pointer))),
        ObjectType.Module => NwModule.Instance,
        ObjectType.Area => NwArea.CreateInternal(CNWSArea.FromPointer(NWNXUtils.AsNWSArea(gameObject.Pointer))),
        ObjectType.Trigger => NwTrigger.CreateInternal(CNWSTrigger.FromPointer(NWNXUtils.AsNWSTrigger(gameObject.Pointer))),
        ObjectType.Door => NwDoor.CreateInternal(CNWSDoor.FromPointer(NWNXUtils.AsNWSDoor(gameObject.Pointer))),
        ObjectType.Waypoint => NwWaypoint.CreateInternal(CNWSWaypoint.FromPointer(NWNXUtils.AsNWSWaypoint(gameObject.Pointer))),
        ObjectType.Encounter => NwEncounter.CreateInternal(CNWSEncounter.FromPointer(NWNXUtils.AsNWSEncounter(gameObject.Pointer))),
        ObjectType.Store => NwStore.CreateInternal(CNWSStore.FromPointer(NWNXUtils.AsNWSStore(gameObject.Pointer))),
        ObjectType.Sound => NwSound.CreateInternal(CNWSSoundObject.FromPointer(NWNXUtils.AsNWSSoundObject(gameObject.Pointer))),
        ObjectType.AreaOfEffect => NwAreaOfEffect.CreateInternal(CNWSAreaOfEffectObject.FromPointer(NWNXUtils.AsNWSAreaOfEffectObject(gameObject.Pointer))),
        _ => null,
      };
    }
