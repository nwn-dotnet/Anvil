using System;
using NWN.Core;

namespace Anvil.API
{
  /// <summary>
  /// Represents a talent structure. A talent is a feat, skill or spell possessed by a creature.
  /// </summary>
  public sealed partial class Talent : EngineStructure
  {
    internal Talent(IntPtr handle, bool memoryOwn) : base(handle, memoryOwn) {}

    /// <summary>
    /// Gets the associated feat, if this talent is a feat.
    /// </summary>
    /// <exception cref="InvalidOperationException">Thrown if this talent is not a feat.</exception>
    public NwFeat Feat => NwFeat.FromFeatId(TryGetId(TalentType.Feat))!;

    /// <summary>
    /// Gets the associated skill, if this talent is a skill.
    /// </summary>
    /// <exception cref="InvalidOperationException">Thrown if this talent is not a skill.</exception>
    public NwSkill Skill => NwSkill.FromSkillId(TryGetId(TalentType.Skill))!;

    /// <summary>
    /// Gets the associated spell, if this talent is a spell.
    /// </summary>
    /// <exception cref="InvalidOperationException">Thrown if this talent is not a spell.</exception>
    public NwSpell Spell => NwSpell.FromSpellId(TryGetId(TalentType.Spell))!;

    /// <summary>
    /// Gets the type of this talent (Spell/Feat/Skill).
    /// </summary>
    public TalentType Type => (TalentType)NWScript.GetTypeFromTalent(this);

    /// <summary>
    /// Gets a value indicating whether this talent is valid.
    /// </summary>
    public bool Valid => NWScript.GetIsTalentValid(this).ToBool();

    private protected override int StructureId => NWScript.ENGINE_STRUCTURE_TALENT;

    /// <summary>
    /// Converts a native pointer to a <see cref="Talent"/> engine structure.
    /// </summary>
    /// <param name="intPtr">The native pointer to the talent.</param>
    /// <returns>A <see cref="Talent"/> wrapping the specified pointer.</returns>
    public static implicit operator Talent(IntPtr intPtr)
    {
      return new Talent(intPtr, true);
    }

    private int TryGetId(TalentType expectedType)
    {
      if (Type != expectedType)
      {
        throw new InvalidOperationException($"Expected talent to be {expectedType}, but it is {Type}!");
      }

      return NWScript.GetIdFromTalent(this);
    }
  }
}
